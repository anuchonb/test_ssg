...
function changeStatus(caseId, currentStatus) {
    const newStatus = prompt('เปลี่ยนสถานะเคส #' + caseId + '\nสถานะปัจจุบัน: ' + (currentStatus || 'ไม่ระบุ') + '\n\nกรอกสถานะใหม่:\n(ส่งเคส, กำลังติดตาม, อนุมัติ, ยกเลิก, ไม่สนใจ, วงเงินไม่ถึง)', currentStatus);
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.success) {
                    ...
                    alert('ผิดพลาด: ' + (response.message || 'ไม่สามารถเปลี่ยนสถานะได้'));
                }
            },
            error: function() {
